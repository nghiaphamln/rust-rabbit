...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let exp_handle = tokio::spawn(async move {
        exponential_consumer.consume(|task: ProcessingTask| async move {
            info!("Exponential retry - Processing task {}: {}", task.id, task.task_type);
            
            match simulate_task_processing(&task, "exponential").await {
                Ok(_) => Ok(()),
                Err(e) => {
                    warn!("Exponential task {} failed: {} (will retry with exponential backoff)", task.id, e);
                    Err(e)
                }
            }
        }).await
    ...
    let linear_handle = tokio::spawn(async move {
        linear_consumer.consume(|task: ProcessingTask| async move {
            info!("Linear retry - Processing task {}: {}", task.id, task.task_type);
            
            match simulate_task_processing(&task, "linear").await {
                Ok(_) => Ok(()),
                Err(e) => {
                    warn!("Linear task {} failed: {} (will retry in 10 seconds)", task.id, e);
                    Err(e)
                }
            }
        }).await
    ...
    let custom_handle = tokio::spawn(async move {
        custom_consumer.consume(|task: ProcessingTask| async move {
            info!("Custom retry - Processing task {}: {}", task.id, task.task_type);
            
            match simulate_task_processing(&task, "custom").await {
                Ok(_) => Ok(()),
                Err(e) => {
                    warn!("Custom task {} failed: {} (will retry with custom delays)", task.id, e);
                    Err(e)
                }
            }
        }).await
    ...
    let fast_handle = tokio::spawn(async move {
        fast_consumer.consume(|task: ProcessingTask| async move {
            info!("Fast retry - Processing task {}: {}", task.id, task.task_type);
    ...
    let conservative_handle = tokio::spawn(async move {
        conservative_consumer.consume(|task: ProcessingTask| async move {
            info!("Conservative retry - Processing task {}: {}", task.id, task.task_type);
            
            match simulate_task_processing(&task, "conservative").await {
                Ok(_) => Ok(()),
                Err(e) => {
                    warn!("Conservative task {} failed: {} (will retry after long delay)", task.id, e);
                    Err(e)
                }
            }
        }).await
    ...
    let no_retry_handle = tokio::spawn(async move {
        no_retry_consumer.consume(|task: ProcessingTask| async move {
    ...
    let selective_handle = tokio::spawn(async move {
        selective_consumer.consume(|task: ProcessingTask| async move {
            info!("Selective retry - Processing task {}: {}", task.id, task.task_type);
            ...
            match simulate_task_processing(&task, "selective").await {
                ...
                Err(e) => {
                    ...
                    if is_retryable_error(&e) {
                        warn!("Selective task {} failed with retryable error: {}", task.id, e);
                        Err(e) // Will retry
                    } else {
                        error!("Selective task {} failed with permanent error: {}", task.id, e);
...
async fn simulate_task_processing(
    ...
    // Success case
    info!("Task {} ({}) completed successfully with {} retry strategy", 
...
mod tests {
    ...
    fn test_retry_delay_calculation() {
        ...
        assert_eq!(linear_config.calculate_delay(0), Some(Duration::from_secs(10)));
        assert_eq!(linear_config.calculate_delay(1), Some(Duration::from_secs(10)));
        assert_eq!(linear_config.calculate_delay(2), Some(Duration::from_secs(10)));
