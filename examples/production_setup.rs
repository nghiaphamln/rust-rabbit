...
async fn main() -> Result<()> {
    ...
    let _notification_handle = tokio::spawn(start_notification_processor(connection.clone(), stats.clone()));
...
async fn start_order_processor(
    ...
    consumer.consume(move |msg: rust_rabbit::Message<OrderEvent>| {
...
async fn start_notification_processor(
    ...
    consumer.consume(move |msg: rust_rabbit::Message<NotificationTask>| {
        let stats = Arc::clone(&stats);
        async move {
...
async fn start_message_producer(connection: Arc<Connection>) -> Result<()> {
    ...
    loop {
        ...
        if let Err(e) = publisher
            .publish_to_exchange("notifications", "notification.order", &notification, options)
            .await
        {
