    ...
    loop {
        match tokio::time::timeout(Duration::from_secs(timeout), Connection::new(url)).await {
            ...
            Err(_) => {
                ...
                let delay = Duration::from_secs(2_u64.pow(current_retry));
                warn!(
                    "Connection timeout on attempt {}. Retrying in {:?}",
                    current_retry, delay
                );
                tokio::time::sleep(delay).await;
            }
    ...
    match order.event_type {
        OrderEventType::Created => {
            let notification = NotificationTask {
                ...
                message: format!(
                    "Your order {} has been created for ${:.2}",
        ...
        _ => {
            debug!(
                "No notification needed for order event type: {:?}",
                order.event_type
            );
        }
    }
