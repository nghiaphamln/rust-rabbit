...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let simple_handle = tokio::spawn(async move {
        simple_consumer.consume(|order: Order| async move {
            info!("Processing simple order: {} for customer {} (${:.2})", 
    ...
    let retry_handle = tokio::spawn(async move {
        retry_consumer.consume(|order: Order| async move {
            info!("Processing order with retry: {} (${:.2})", order.id, order.amount);
    ...
    let exchange_handle = tokio::spawn(async move {
        exchange_consumer.consume(|notification: Notification| async move {
            info!("Processing notification: {} (priority: {})", 
    ...
    let custom_handle = tokio::spawn(async move {
        custom_consumer.consume(|request: serde_json::Value| async move {
    ...
    let batch_consumer = Consumer::builder(connection.clone(), "batch_orders")
        .retry(RetryConfig::exponential(3, Duration::from_millis(500), Duration::from_secs(30)))
        .concurrency(1) // Process one batch at a time
        .build()
        .await?;

    let batch_handle = tokio::spawn(async move {
        batch_consumer.consume(|order: Order| async move {
...
async fn send_notification(notification: &Notification) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
mod tests {
    ...
    fn test_error_classification() {
        ...
        let permanent_error: Box<dyn std::error::Error + Send + Sync> = "Invalid data format".into();
