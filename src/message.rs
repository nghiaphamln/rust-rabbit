...
impl<T> MessageEnvelope<T> {
    ...
    pub fn with_header(mut self, key: &str, value: &str) -> Self {
        self.metadata.headers.insert(key.to_string(), value.to_string());
        self
    }
...
mod tests {
    ...
    fn test_message_envelope_creation() {
        ...
        let envelope = MessageEnvelope::new(payload.clone(), "test_queue")
            .with_max_retries(3); // Set max retries so it's not exhausted initially
    ...
    fn test_failure_summary() {
        ...
        let envelope = MessageEnvelope::new(payload, "test_queue")
            .with_max_retries(2)
            .with_error("Database connection failed", ErrorType::Transient, Some("Timeout after 5s"))
